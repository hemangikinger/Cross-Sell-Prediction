...
vehicle$Region_Code <-ifelse(vehicle$Region_Code %in% c(9,23,25,33,44,50,34,36,42), 1, 
                             ifelse(vehicle$Region_Code %in% c(18,17,26,39,55,19,20,27,29,31,38,46), 2,
                                    ifelse(vehicle$Region_Code %in% c(10,11,12,13,24,37,45,51,54,1,21,28,47,5,22,40,48),3,
                                           ifelse(vehicle$Region_Code %in% c(4,8,16,35,30,49,32,56,2,6,15,41,53),4,5))))
...
vehicle_train = vehicle[1:10]
vehicle_test = vehicle[6:10]

vehicle.trainLabels <- vehicle$Response[1:5]
vehicle.testLabels <- vehicle$Response[6:10]

library(class)
vehicle_pred <- knn(train = vehicle_train, test = vehicle_test, cl=vehicle.trainLabels, k=2)
tb <- table(vehicle_pred,vehicle.testLabels)
...
# vehicle.Train=vehicle[train,c(2,3,6,7,8)]
# vehicle.Test=vehicle[-train,c(2,3,6,7,8)]
# vehicle.trainLabels <- vehicle$Response[train]
# vehicle.testLabels <- vehicle$Response[-train]

# library(class)
# vehicle_pred <- knn(train = vehicle.Train, test = vehicle.Test, cl=vehicle.trainLabels, k=3)
# tb <- table(vehicle_pred,vehicle.testLabels)
# accuracy <- function(x){sum(diag(x)/(sum(rowSums(x)))) * 100}
# accuracy(tb)
```
...
data_balanced_over <- ovun.sample(Response ~ ., data = vehicle, method = "over",N = 668798)$data
# data_balanced_over <- ovun.sample(Response ~ ., data = vehicle, method = "over",N = 668798)$data
#381109
#287689
#668798
...
data_balanced_under <- ovun.sample(Response ~ ., data = vehicle, method = "under", N = 93420)$data
table(data_balanced_under$Response)
```

```{r echo=TRUE, message=FALSE, warning=FALSE, , echo=FALSE}

n = nrow(data_balanced_under)

label = as.integer(data_balanced_under$Response)-1

train.index=sample(1:n,size=round(n*0.70),replace=FALSE)
train.data=data_balanced_under[train.index,]
train.label = label[train.index]
test.data = data_balanced_under[-train.index,]
test.label = label[-train.index]


library(class)
vehicle_pred <- knn(train = train.data, test = test.data, cl=train.label, k=3)
tb <- table(vehicle_pred,test.label)
accuracy <- function(x){sum(diag(x)/(sum(rowSums(x)))) * 100}
accuracy(tb)

# n=dim(data_balanced_over)[1]
# set.seed(123)
# train=sample(1:n,size=round(n*0.75),replace=FALSE)
# vehicle.Train=data_balanced_over[train,c(2,3,6,7,8)]
# vehicle.Test=data_balanced_over[-train,c(2,3,6,7,8)]
# vehicle.trainLabels <- data_balanced_over$Response[train]
# vehicle.testLabels <- data_balanced_over$Response[-train]
# 
# library(class)
# vehicle_pred <- knn(train = vehicle.Train, test = vehicle.Test, cl=vehicle.trainLabels, k=1000,prob= TRUE)
# tb <- table(vehicle_pred,vehicle.testLabels)
# accuracy <- function(x){sum(diag(x)/(sum(rowSums(x)))) * 100}
# accuracy(tb)
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE, , echo=FALSE}
n = nrow(data_balanced_over)

label = as.integer(data_balanced_over$Response)-1

train.index=sample(1:n,size=round(n*0.70),replace=FALSE)
train.data=data_balanced_over[train.index,]
train.label = label[train.index]
test.data = data_balanced_under[-train.index,]
test.label = label[-train.index]
